...
class SmartStockApp {
                    ...
                    <tbody>
                        ${state.transactions.map(t => `
                            <tr>
                                ...
                                <td>${t.items ? t.items.map(i => `${i.nama} (x${i.qty})`).join('<br>') : '-'}</td>
                                <td>Rp ${t.totalJual ? t.totalJual.toLocaleString('id-ID') : 0}</td>
                                <td style="color:var(--success); font-weight:bold;">+ Rp ${((t.totalJual || 0) - (t.totalModal || 0)).toLocaleString('id-ID')}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>`}
        ...
        el.innerHTML = `
            ...
            <div class="grid-2">
                <div class="card">
                    ...
                    <button class="btn-success" onclick="app.simpanBarangGudang()" style="width:100%">Simpan ke Database Cloud</button>
                </div>
                ...
                <div class="card" style="max-height: 500px; overflow-y: auto;">
                    ...
                    <table id="gudang_table">
                        ...
                        <tbody>
                            ${state.inventory.length === 0 ? '<tr><td colspan="4">Data masih kosong.</td></tr>' : state.inventory.map(b => `<tr><td><strong>${b.id}</strong></td><td>${b.nama}<br><small style="color:gray">${b.spesifik}</small></td><td>${b.stock}</td><td>Rp ${b.jual.toLocaleString('id-ID')}</td></tr>`).join('')}
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    simpanBarangGudang() {
        ...
        let itemsByPrefix = state.inventory.filter(i => i.id && i.id.startsWith(prefix));
        ...
        // 1. KIRIM DATA KE CLOUD FIREBASE
        db.ref('inventory/' + newId).set({
            ...
            alert(`Berhasil! Barang tersimpan di Cloud.\nID Barang: ${newId}`);
            // Form input otomatis dikosongkan setelah sukses
            document.getElementById('g_nama').value = '';
            document.getElementById('g_stok').value = '0';
            document.getElementById('g_modal').value = '0';
            document.getElementById('g_jual').value = '0';
            document.getElementById('g_spesifik').value = '';
        }).catch((error) => {
    ...
    cariBarangKasir() {
        ...
        if (barang) {
            ...
            alert("Barang tidak ditemukan di Database Cloud!");
            resArea.style.display = 'none';
        }
    }
    ...
    prosesTransaksi() {
        ...
        db.ref('transactions/' + notaId).set(dataTransaksi).then(() => {
            ...
            // 2. KURANGI STOK BARANG DI CLOUD SECARA OTOMATIS
            state.cart.forEach(c => {
                let inventoryItem = state.inventory.find(i => i.id === c.id);
                if(inventoryItem) {
                    let sisaStok = inventoryItem.stock - c.qty;
                    db.ref('inventory/' + c.id + '/stock').set(sisaStok);
                }
            });

            alert(`Transaksi Sukses!\nData tersimpan di Cloud.\nTotal Transaksi: Rp ${totalJual.toLocaleString('id-ID')}`);
            state.cart = []; // Kosongkan keranjang
            this.renderDashboard(); // Refresh tampilan
        }).catch(err => {
